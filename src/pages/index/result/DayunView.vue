  ...
  <view class="bazi-container">
    ...
    <view class="horizontal-scroll">
      <text style="font-size: 24rpx; margin-top: 8rpx; margin-bottom: 8rpx">{{
        analysisResult.data.ret_Info.basic_analysis_1
      }}</text>
    </view>
    <view class="horizontal-scroll">
      <view
      ...
      <view class="horizontal-scroll">
        <view
          v-for="(dayun, index) in dayunArr"
          :key="index"
          class="dayun-tab"
          :class="{ active: activeDayunIndex === index }"
          @click="
            () => {
        ...
        >
          ...
          <view v-if="index === 0">
            <view style="font-size: 28rpx"><text>小 </text></view>
    ...
    <view class="horizontal-scroll">
      ...
      <view class="horizontal-scroll">
        <view
          v-for="(liunian, liuIndex) in currentDayun[5]"
          :key="liuIndex"
          class="dayun-tab"
...
const currentDayun = computed(() => {
  // 确保数组有数据且索引有效
  if (
    props.dayunArr.length > 0 &&
    activeDayunIndex.value < props.dayunArr.length
  ) {
